...
data = apdt.io.load_nms(site_list, '2017-01-01', '2017-06-30')
...
# Construct dataset
dataset = apdt.ml.DataSet(data, method='window', seq_len=244)

# Define the model structure
class MyModel(apdt.ml.TFModel):
    def def_model(self, **kwarg):
        self.input = tf.placeholder(tf.float32, shape=(len(site_list), 244, 1))
